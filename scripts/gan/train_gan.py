#!/usr/bin/env python
# scripts/gan/train_gan.py
# WGAN-GP trainer for tabular malware features
# Author: Callum Musselwhite
# Last edit: 2025-09-17
# Purpose: train a generator (and discriminator) on selected rows, optionally malware-only, and save the generator state dict
...
def main():
    ...
    if args.indices_json:
        ...
        try:
            ...
            # handle simple index JSONs we write in experiments
            if "train_only" in d:
            ...
            # fall back to full SplitIndices format
    ...
    # malware-only subset if requested
    ...
    # standardize features using only selected training rows
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X_all.astype(np.float32))
    X_tensor = torch.from_numpy(X_scaled)  # kept on CPU; batches moved to device

    # dataloader for SGD loops
    loader = DataLoader(
        TensorDataset(X_tensor),
        batch_size=args.batch_size,
        shuffle=True,
        drop_last=True,          # improves stability for some losses
        num_workers=0,           # 0 is typically best on macOS
    ...
    # optimizers with WGAN-GP friendly betas
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (real_batch,) in loader:
            ...
            # train discriminator n_critic times per batch
    ...
    # save only the generator to the requested location or default
