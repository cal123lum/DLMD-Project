#!/usr/bin/env python
# scripts/experiments/holdouts/run_scarcity_sweep.py
# Generic sweep runner for temporal or family holdouts
# Author: Callum Musselwhite
# Last edit: 2025-09-17
# Purpose: sweep fractions × seeds across methods by calling eval_holdout and append results to a CSV
...
def run_eval(kind_flag: str, tag: str, extra_args: list[str]) -> None:
    # call eval_holdout with the given flags
...
def load_metric_json(kind: str, tag: str, metrics_subdir: str = "") -> dict | None:
    # metrics may appear with or without a legacy leading underscore in the tag
...
def main():
    ...
    ap.add_argument("--seeds", type=str, default=None,
                    help="comma list overriding --seed, e.g., 42,1337,2025")

    ap.add_argument("--tag-prefix", type=str, required=True)
    ap.add_argument("--out-csv", type=str, default=None)

    # model and eval knobs
    ...
    # GAN quality knobs passed to eval_holdout
    ...
    # rolling windows for temporal sweeps
    ...
    # build rolling windows for temporal if requested
    ...
    # flags common to every eval call
    common = []
    ...
    for w_i, (wstart, wend) in enumerate(windows):
        for seed in seeds:
            for f in fracs:
                ...
                frac_tag = frac_str.replace(".", "")  # 0.002 -> 0002
                ...
                if "gan" in methods:
                    if not args.gan_generator:
                        print("[warn] compare includes 'gan' but --gan-generator is missing, skipping")
                    else:
