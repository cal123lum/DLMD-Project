...
# GAN training subset selector for malware positives
# Author: Callum Musselwhite
# Last edit: 2025-09-17
# Purpose: select a subset of positive-class train rows for GAN training with optional family stratification
...
def main():
    ...
    ap.add_argument("--stratify-by-family", action="store_true",
                    help="sample malware across many families")
    ...
    # optional cap with or without family stratification
    if args.max_train_rows is not None and len(mal_idx) > args.max_train_rows:
        ...
        if args.stratify_by_family:
            ...
            # proportional allocation with a per-family floor
            per_fam = np.maximum(
                1,
            ...
            # adjust for rounding to hit the exact cap if possible
