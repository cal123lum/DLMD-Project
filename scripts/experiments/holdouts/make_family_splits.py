#!/usr/bin/env python3
# scripts/experiments/holdouts/make_family_splits.py
# Family LOFO split generator with benign hashing
# Author: Callum Musselwhite
# Last edit: 2025-09-17
# Purpose: create per-family holdout splits with a deterministic benign test sample

import json, hashlib, numpy as np, pandas as pd
from pathlib import Path

META_CSV = Path("data/raw/bodmas_metadata.csv")
NPZ = Path("data/raw/bodmas.npz")
OUT_DIR = Path("data/processed/splits/family_lofo")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# settings
MIN_TEST_POS = 200        # minimum positives required in the held-out family
BENIGN_TEST_FRAC = 0.20   # fraction of benigns routed to test via hash
...
neg_test_idx = neg_idx[neg_test_mask]
neg_train_idx = neg_idx[~neg_test_mask]

# iterate families excluding UNKNOWN
