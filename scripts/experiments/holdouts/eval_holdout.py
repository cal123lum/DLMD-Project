#!/usr/bin/env python3
# scripts/experiments/holdouts/eval_holdout.py
# Malware augmentation and evaluation pipeline
# Author: Callum Musselwhite
# Last edit: 2025-09-17
# Purpose: evaluate RF on temporal or family splits with optional augmentation (GAN, EVO, ROS, SMOTE) and validation thresholding
...
from sklearn.preprocessing import StandardScaler  # retained import
...
def audit_splits_and_aug(
    ...
    # validation must be subset of train (val indices refer to real rows within train)
...
def _nn_min_dist(A, B, k=1):
    """distance to nearest member of B for each row in A"""
...
def add_gan_synth(
    ...
    """
    Generate n_synth malware-like samples with a trained generator and append to (X_real, y_real)
    quality:
      - 'none'        take first n_synth
      - 'nn'          keep closest to positive manifold
      - 'nn_boundary' near negative boundary but still close to positives
    """
    ...
    # cached generator/scaler path handling
...
def add_evo_synth(
    ...
    """
    Lightweight evolutionary synthesizer in feature space
    child = convex(p1, p2) + Gaussian mutation, clamped to robust per-feature range
    uses same quality filters as GAN
    """
    ...
    # robust per-feature ranges and scale
    ...
    # mutation scaled by robust spread
...
def _evo_refine_pool(candidates, X_like_pos, *, seed, parent_source="both",
    ...
    One evolutionary step over candidates using parents from:
      ...
      'both' concatenation of candidates and X_like_pos
    child = convex(parent1, parent2) + Gaussian mutation, clamped to robust range
    """
    if len(candidates) == 0:
        return candidates
    rng = np.random.default_rng(seed)
...
def oversample_to_const(X, y, target_n, seed=42):
    """duplicate positives with replacement until len == target_n"""
...
def smote_to_const(X, y, target_n, seed=42, k=5):
    """
    small SMOTE variant for numeric features
    falls back to oversampling when positives are too few to interpolate
    """
    if target_n is None or target_n <= len(y):
        return X, y
    ...
    # neighbors chosen within positives only
...
def main():
    ...
    ap.add_argument("--gan-quality", choices=["none", "nn", "nn_boundary"], default="nn",
                    help="quality filter for synth")
    ap.add_argument("--gan-qmult", type=float, default=3.0,
                    help="oversample factor before quality filtering")
    ...
    ap.add_argument("--val-threshold", choices=["none", "f1", "mcc", "balacc"], default="balacc",
                    help="pick decision threshold on a small validation split")
    ...
    ap.add_argument("--balance-after-augment", action="store_true",
                    help="downsample the majority class after augmentation")
    ap.add_argument("--balance-ratio", type=float, default=1.0,
                    help="cap majority/minority ratio after augmentation")
    ap.add_argument("--metrics-subdir", type=str, default="",
                    help="write under data/processed/metrics/<subdir>/")
    ...
    ap.add_argument("--evo-parent-source", choices=["gan", "real", "both"], default="both",
                    help="parent pool for refinement")
    ...
    # variant string for bookkeeping
    variant = "real"
    if args.use_gan:
        variant = "evogan" if args.gan_evo_refine else "gan"
    ...
    # optional time window on test
    test_mask = None
    if args.test_start or args.test_end:
        te_ts = meta.loc[test_idx_full, "timestamp"]
    ...
    # scarce subset with floors on both classes
    ...
    if args.tune:
        ...
        clf = RandomForestClassifier(
            ...
            n_jobs=args.rf_max_depth,  # note: n_jobs tied to rf_max_depth arg
    ...
    # build validation split from augmented train and select threshold if requested
    if args.val_threshold != "none":
        ...
        # map real part of val indices to global indices for audit
