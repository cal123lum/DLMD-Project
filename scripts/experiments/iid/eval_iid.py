#!/usr/bin/env python
# scripts/experiments/iid/eval_iid.py
# IID experiment runner with optional scarcity and augmentation
# Author: Callum Musselwhite
# Last edit: 2025-09-17
# Purpose: split IID, optionally induce scarcity and augment (ROS/SMOTE/GAN), train RF with optional val-threshold, save metrics and preds
...
def gan_generate(G: Generator, n_samples: int, seed: int) -> np.ndarray:
    # sample latent noise and generate n_samples feature vectors
    if n_samples <= 0:
...
def load_generator(gen_path: str) -> Generator:
    # load a CPU generator from a .pth state dict
    G = Generator().to("cpu")
...
def oversample_to_const(X, y, target_n, seed=42):
    # duplicate positives with replacement until dataset reaches target_n
...
def smote_to_const(X, y, target_n, seed=42, k=5):
    # numeric-only SMOTE with a safe k, falls back to ROS if too few positives
    if target_n is None or target_n <= len(y):
        return X, y, 0
    rng = np.random.default_rng(seed)
...
def pick_threshold(y_true, proba, which="balacc", grid=200):
    # pick a decision threshold on a grid using a chosen metric
...
def main():
    ...
    # 1) load NPZ and split IID with stratification
    ...
    # 2) induce scarcity if requested, with floors for both classes
    if args.scarce_real_frac is not None:
        ...
        while (n_pos + n_neg) < n_total and (n_pos < len(idx_pos) or n_neg < len(idx_neg)):
            if n_pos < len(idx_pos):
                n_pos += 1
            if (n_pos + n_neg) < n_total and n_neg < len(idx_neg):
                n_neg += 1
    ...
    # 3) optional augmentation to a constant size
    method = args.method
    if args.const_train_size is not None and args.const_train_size > len(ytr_small):
        if method == "ros":
            ...
            cap = int(args.gan_synth_per_real * max(1, n_real_pos))  # guard against runaway synth
    ...
    # 4) train RF with optional validation threshold selection
